...
function print_new_array() {
  ...
  url => {
    ...
    "payload": JSON.stringify([{'keyword':url, 'language_code': 'en', 'location_code': 2840}])
  }
  });
  return new_array;
}
...
function show_results() {
  ...
  const reframed = new_results.map(result => JSON.parse(result.getContentText()));
  ...
  const results_array = reframed.map(req => {
    if (req['tasks'][0]['result'][0]['items_count'] > 0) {
      if (req['tasks'][0]['data']['keyword'] == req['tasks'][0]['result'][0]['items'][0]['url']) {
        return ["Indexed"]
      } else {
        return ["Not Indexed"]
      }
  ...
  sheet.getRange(2,lastColumn + 1,results_array.length,results_array[0].length).setValues(results_array);
